...
import { Settings, CheckCircle, Key } from "lucide-react";

export default function SettingsPage() {
  const [form, setForm] = useState({
    deepSeekApiKey: '',
  });
  const [status, setStatus] = useState({ hasDeepSeek: false });
  const [saved, setSaved] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/settings').then(r => r.json()).then(d => {
      setStatus({ hasDeepSeek: d.hasDeepSeek });
      setLoading(false);
    });
  }, []);
      ...
      <div className="max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-10">
          ...
          <div>
            ...
            <p className="text-neutral-500 text-sm">Configure Core Evasion Engine API Keys</p>
          </div>
        </div>
